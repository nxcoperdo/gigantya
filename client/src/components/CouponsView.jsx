...
function CouponModal({ isOpen, onClose, onSave, coupon, restaurant }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-xl overflow-hidden animate-scaleUp">
        <div className="p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 bg-primary/10 text-primary rounded-full flex items-center justify-center">
              <Ticket size={20} />
            </div>
            <h3 className="text-xl font-bold text-dark">{coupon ? 'Editar Cupón' : 'Nuevo Cupón'}</h3>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Mín. Compra</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Usos Máximos</label>
                <input
                  type="number"
                  min="1"
...
function CouponCard({ coupon, onEdit, onDelete }) {
  ...
  return (
    <div className={`border rounded-xl p-4 bg-white ${isExpired ? 'opacity-60 bg-gray-50' : 'hover:shadow-sm'} transition-all`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-600 mb-2">
            <div className="flex items-center gap-1">
              ...
              <span className="font-semibold text-dark">{discountLabel} de descuento</span>
            </div>
            {coupon.min_compra && (
              <div className="flex items-center gap-1">
                <DollarSign size={14} />
      ...
      {copied && (
        <div className="mt-2 text-xs text-green-600 font-medium animate-fadeIn">
          ¡Código copiado!
        </div>
      )}
    </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-dark">Gestión de Cupones</h2>
            <p className="text-sm text-gray-600">Crea y administra descuentos para tus clientes</p>
          </div>
        </div>
        <button
          type="button"
      ...
      {loading ? (
        ...
        <div className="card-lg py-12 text-center">
          ...
          <p className="text-gray-600">No tienes cupones creados todavía</p>
          <p className="text-sm text-gray-500 mt-1">Crea tu primer cupón para ofrecer descuentos a tus clientes</p>
        </div>
      ) : (
